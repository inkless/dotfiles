...
function git.cmd(args, ...) return zvim.cmd("git -C " .. zvim.install.home .. " " .. args, ...) end
...
function git.remote_url(remote, ...) return zvim.trim_or_nil(git.cmd("remote get-url " .. remote, ...)) end

function git.current_version(...) return zvim.trim_or_nil(git.cmd("describe --tags", ...)) end

function git.current_branch(...) return zvim.trim_or_nil(git.cmd("rev-parse --abbrev-ref HEAD", ...)) end

function git.local_head(...) return zvim.trim_or_nil(git.cmd("rev-parse HEAD", ...)) end

function git.remote_head(remote, branch, ...)
  return zvim.trim_or_nil(git.cmd("rev-list -n 1 " .. remote .. "/" .. branch, ...))
end

function git.tag_commit(tag, ...) return zvim.trim_or_nil(git.cmd("rev-list -n 1 " .. tag, ...)) end
...
function git.parse_remote_url(str)
  return vim.fn.match(str, zvim.url_matcher) == -1
      and git.url .. str .. (vim.fn.match(str, "/") == -1 and "/zvim.git" or ".git")
    or str
end
