...
local options = zvim.user_plugin_opts(
  "updater",
  { remote = "origin", branch = "main", channel = "nightly", show_changelog = true }
)

if zvim.install.is_stable ~= nil then options.channel = zvim.install.is_stable and "stable" or "nightly" end

zvim.updater = { options = options }
if options.pin_plugins == nil and options.channel == "stable" or options.pin_plugins then
  local loaded, snapshot = pcall(fn.readfile, zvim.install.home .. "/packer_snapshot")
  if loaded then
    loaded, snapshot = pcall(fn.json_decode, snapshot)
    zvim.updater.snapshot = type(snapshot) == "table" and snapshot or nil
  end
  if not loaded then vim.api.nvim_err_writeln "Error loading packer snapshot" end
end

function zvim.updater.version()
  local version = zvim.install.version or git.current_version(false)
  if version then zvim.notify("Version: " .. version) end
end
...
function zvim.updater.update()
  if not git.is_repo() then
    zvim.notify("Updater not available for non-git installations", "error")
    return
  end
  ...
  then
    ...
    if #breaking > 0 and not options.skip_prompts and not zvim.confirm_prompt(breaking_prompt) then
      zvim.echo(cancelled_message)
      return
    end
    ...
    local summary = {
      { "zvim updated successfully to ", "Title" },
