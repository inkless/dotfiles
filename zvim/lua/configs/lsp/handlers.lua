...
local function get_buf_opts(buffer, desc) return { noremap = true, silent = true, buffer = buffer, desc = desc } end
...
zvim.lsp.on_attach = function(client, bufnr)
  ...
  map("n", "<leader>cf", "<cmd>Format<cr>", get_buf_opts(bufnr, "Format code"))
  ...
  if vim.g.lsp_qf_list == "telescope" then
    ...
    map(
      ...
      get_buf_opts(bufnr, "References of current symbol")
    )
    map(
      "n",
      "gD",
      function() require("telescope.builtin").lsp_type_definitions() end,
      get_buf_opts(bufnr, "Declaration of current symbol")
    )
  end
  ...
  if aerial_avail then aerial.on_attach(client, bufnr) end
...
function zvim.lsp.server_settings(server_name)
  ...
  opts.on_attach = function(client, bufnr)
    if type(server.on_attach) == "function" then server.on_attach(client, bufnr) end

    zvim.lsp.on_attach(client, bufnr)

    if type(opts_on_attach) == "function" then opts_on_attach(client, bufnr) end
  end
  return opts
end
